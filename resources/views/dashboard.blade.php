            ...
            <div class="p-6">
                        ...
                        <table class="min-w-full text-left">
                            ...
                            <tbody class="text-gray-600 text-sm font-light">
                                ...
                                <tr class="border-b border-gray-100 hover:bg-gray-50 transition">
                                    ...
                                    <td class="py-4 px-4">
                                        <div class="flex items-center">
                                            <div class="w-10 h-10 rounded-full overflow-hidden mr-3 border border-gray-200">
                                                <img class="w-full h-full object-cover" src="https://ui-avatars.com/api/?name={{ urlencode($app->doctor->name) }}&background=random"/>
                                            </div>
                                            <div>
                                                <span class="font-bold text-gray-800 block">{{ $app->doctor->name }}</span>
                                                <span class="text-xs text-gray-500">{{ $app->doctor->specialty->name ?? 'Chuyên khoa' }}</span>
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    <td class="py-4 px-4 text-center">
                                        @if($app->status == 'pending')
                                            <form action="{{ route('booking.cancel', $app->id) }}" method="POST" onsubmit="return confirm('Bạn có chắc muốn hủy lịch này không?');">
                                                @csrf
                ...
                @else
                    <div class="text-center py-16">
                        ...
                        <a href="{{ route('home') }}" class="bg-medpro text-white px-6 py-2.5 rounded-full font-bold hover:opacity-90 transition shadow-lg shadow-blue-200">
                            Đặt khám ngay
                        </a>
                    </div>
                @endif
