...
<head>
    ...
    <title>{{ $currentType }} - Medpro</title>
    ...
    <div class="bg-white pb-8 pt-4">
        <div class="container mx-auto px-4">
            <div class="text-sm text-gray-500 mb-6 flex gap-2 items-center">
                ...
                <span class="font-bold text-gray-800">{{ $currentType }}</span>
            </div>

            <div class="text-center max-w-3xl mx-auto">
                <h1 class="text-3xl md:text-4xl font-extrabold text-cyan-500 mb-3">{{ $currentType }}</h1>
                <p class="text-gray-500 mb-8">Đặt khám dễ dàng, không lo chờ đợi tại các {{ Str::lower($currentType) }} hàng đầu Việt Nam</p>
        ...
        <div class="container mx-auto px-4">
            <div class="flex gap-3 overflow-x-auto py-4 scrollbar-hide whitespace-nowrap">
                @foreach($typeCounts as $typeName => $count)
                    <a href="{{ route('search', ['type' => $typeName]) }}" 
                       class="px-5 py-2 rounded-full text-sm font-bold transition flex items-center gap-2 
                       {{ $currentType == $typeName ? 'bg-cyan-500 text-white shadow-lg shadow-cyan-200' : 'bg-gray-100 text-gray-600 hover:bg-gray-200' }}">
                        {{ $typeName }} 
                        <span class="{{ $currentType == $typeName ? 'text-cyan-100' : 'text-gray-400' }}">({{ $count }})</span>
                    </a>
                @endforeach
            </div>
        </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="bg-white rounded-2xl p-5 shadow-sm border border-gray-100 hover:shadow-md transition flex flex-col sm:flex-row gap-5 group">
                    ...
                    <div class="w-full sm:w-32 h-32 flex-shrink-0 flex items-center justify-center bg-white rounded-xl border border-gray-100 p-2">
                        <img src="{{ $hospital->logo ? asset('storage/' . $hospital->logo) : 'https://medpro.vn/_next/image?url=https%3A%2F%2Fcdn-pkh.longvan.net%2Fprod-partner%2F08e3e077-c744-4d97-af9a-5283d7259035-20231225_092222.png&w=128&q=75' }}" 
                             alt="{{ $hospital->name }}" 
                    ...
                    <div class="flex-1 flex flex-col justify-between">
                        ...
                        <a href="{{ route('hospital.show', $hospital->id) }}" class="flex-1 bg-white border border-cyan-500 text-cyan-500 py-2 rounded-full text-sm font-bold hover:bg-cyan-50 transition text-center">
    Xem chi tiết
</a>
            ...
            <div class="mt-8">
                {{ $hospitals->appends(['type' => $currentType, 'keyword' => $keyword])->links() }}
            </div>
        @else
